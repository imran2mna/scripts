...
tar -xvzf cacti-${CACTI_VERSION}.tar.gz -C ${INSTALL_DIR}/


#cp  ${W_DIR}/index.php /var/www/html/
...
#semanage fcontext -a -t httpd_sys_rw_content_t "/var/www/html/cacti/cache(/.*)?"

semanage fcontext -a -t httpd_sys_content_t "${INSTALL_DIR}/cacti-${CACTI_VERSION}(/.*)?"
semanage fcontext -a -t httpd_sys_rw_content_t "${INSTALL_DIR}/cacti-${CACTI_VERSION}/rra(/.*)?"
semanage fcontext -a -t httpd_sys_rw_content_t "${INSTALL_DIR}/cacti-${CACTI_VERSION}/log(/.*)?"
semanage fcontext -a -t httpd_sys_rw_content_t "${INSTALL_DIR}/cacti-${CACTI_VERSION}/cache(/.*)?"

# Writable during installation only
semanage fcontext -a -t httpd_sys_rw_content_t "${INSTALL_DIR}/cacti-${CACTI_VERSION}/resource(/.*)?"
semanage fcontext -a -t httpd_sys_rw_content_t "${INSTALL_DIR}/cacti-${CACTI_VERSION}/scripts(/.*)?"

grep ${INSTALL_DIR}/cacti-${CACTI_VERSION} /etc/selinux/targeted/contexts/files/file_contexts.local

rmdir /var/www/html || mv /var/www/html{,.old} 
ln -s ${INSTALL_DIR}/cacti-${CACTI_VERSION}/ /var/www/html


#restorecon -Rv /var/www/html/ 
chown -R apache:apache ${INSTALL_DIR}/cacti-${CACTI_VERSION}
restorecon -Rv ${INSTALL_DIR}/cacti-${CACTI_VERSION}/
...
mysql -uroot -p${DB_ROOT_PASSWD} ${DB_NAME} < ${INSTALL_DIR}/cacti-${CACTI_VERSION}/cacti.sql
...
grep ${INSTALL_DIR}/cacti-${CACTI_VERSION}/poller.php /etc/crontab || echo "*/5 * * * *  apache php ${INSTALL_DIR}/cacti-${CACTI_VERSION}/poller.php > /dev/null 2>&1" >> /etc/crontab